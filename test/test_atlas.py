...
# ── The sound sheet, and the second wire on it ──────────────────────────────


def test_python_never_names_a_host_call_that_is_not_there():
    """The C host is the project's other seam, and it is asymmetric on
    purpose: `host.c` calls itself — `gestate_host_halt` calls
    `gestate_host_unblock` — so a function Python never names is an
    internal, not a dead export.  The other direction is a crash."""
    drift = atlas.host_drift(ROOT)
    assert drift == [], "; ".join(drift)


def test_the_host_check_could_fail():
    """Vacuously true is the failure mode of every derived check here."""
    defined, named = atlas.host_wire(ROOT)
    assert len(defined) >= 20, len(defined)
    assert len(named & defined) >= 20, len(named & defined)


def test_the_signal_vocabulary_is_the_compilers_own():
    """`spec/liveaudio.md`'s argument is that this list is short: an
    oscillator, an envelope, a filter and a noise source are all `scan`.
    The sheet reads the table rather than repeating the claim."""
    from gestate.audiograph import FORMERS, PRIMITIVES

    assert atlas.formers() == sorted(FORMERS)
    assert atlas.primitives() == sorted(PRIMITIVES)
    assert "scan" in atlas.formers() and "feedback" in atlas.formers()
    assert atlas.ir_kinds(ROOT) == ["Const", "Var", "Prim", "Call", "Con",
                                    "Field", "Case", "Let"]


# ── What a shared picture has to say for itself ─────────────────────────────


def test_every_sheet_says_where_it_came_from():
    """**These leave the repository.**  A picture in a chat window has
    no `--check` to run, so it carries the commit it was drawn at and
    the date of that commit — the two facts a shared image loses."""
    for name in sorted(atlas.generate(ROOT)):
        svg = (ROOT / "doc" / "atlas" / name).read_text()
        assert 'class="stamp"' in svg, name


def test_the_stamp_is_not_what_makes_a_sheet_stale():
    """It names the commit, so it differs the moment anything else is
    committed — and a check that failed on that would demand a redraw
    after every commit in the project, which is how a generated file
    becomes one nobody regenerates.  The drawing is compared; the stamp
    is left to say what it says."""
    drawn = atlas.render(ROOT)
    older = drawn.replace(atlas.provenance(ROOT), "gestate 0000000 · 1999-01-01")
    assert older != drawn
    assert atlas._unstamped(older) == atlas._unstamped(drawn)


# ── The score sheet, which sorts a vocabulary a person types ────────────────


def test_every_score_word_is_placed():
    """**A new score operator fails until somebody says what it is for.**

    The signatures are read from `music.ges`; which group a word belongs
    to is a claim about what a person is doing when they reach for it,
    and that is the half no reading recovers.  The `asked` group exists
    precisely so that "not for you" is an answer rather than an
    omission.
    """
    loose = atlas.unsorted_words(ROOT)
    assert loose == [], (
        "these score words have no group — place them in "
        "`gestate/atlas.py`'s MUSIC_WORDS: " + ", ".join(loose))
    gone = atlas.stale_words(ROOT)
    assert gone == [], "grouped words `music.ges` no longer has: " + \
        ", ".join(gone)


def test_the_signatures_are_the_librarys_own():
    """Read through the same reader `doc/ref/` is generated by, so a
    changed type changes the drawing rather than the drawing lying."""
    said = {e.name: e.signature.strip() for e in atlas.music_entries(ROOT)}
    assert said["cycle"] == "cycle : [: a :] -> [: a :]"
    assert said["draw"] == "draw : [: Int :]"
    assert said["hear"].startswith("hear : Chan (List Int)")

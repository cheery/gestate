...
to know why a rule says what it says can now see the argument that
produced it rather than take the summary on trust.

## What they are

| file | what it covers |
|---|---|
| [notes-on-models-and-liveness.md](notes-on-models-and-liveness.md) | does the tree work on smaller models · can the rules be trained into an open model · live rules, and what keeps them from degrading |
| [notes-on-the-name.md](notes-on-the-name.md) | what was actually found, whether it is a discovery, and the name |
...
added at his ask as a continuation; one surname more, cited.  A
third, what the tree is then, followed the same way; two surnames
more, both cited authors.
...
has been asked, so both appear as descriptions — *a friend* and *the
other builder*.  One was the author's own instruction when he handed the
file over; the other is `doc/consent.md`'s rule applied by a session.
...
one about the workers, cited by all four of those — and `notes-on-the-trial.md`,
...
about it, and it came out of these very pages — which is the loop
declaring itself, not escaping.  Read them as argument, not as evidence.

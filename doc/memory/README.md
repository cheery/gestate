...
- [The command language and the model come before any GUI](gui-command-language-first.md) — Henri, 2026-09-07, *it must be stressed*: every gesture has a command under it and the picture is never where the model lives; and 2026-09-09 the same rule with its content — the model is **facts**, the verbs are assert and retract, *the GUI is what falls out* — which removes two of the four hard things and leaves layout and the eye
- [Identity is the model's key, never the picture's index](identity-is-the-models-key.md) — Henri, 2026-09-08: the `.notes` file sorts to its agreed order and a note's identity is (voice, tick, key); a reference declares its kind — a thing, a place, or a set (Kale, arXiv 2608.26345) — and a mark carried through edits is a mechanism, never a rule shown to a person
...
- [Declare, hold to parity, derive](declare-parity-derive.md) — how a load-bearing mechanism is replaced here: the parity step makes it safe and the **derivation** is what finds the defect, because a declaration checked against the one file that exists is checked against a witness and not a boundary (2026-09-09, `By "section"` where the parser sorted by the file's own order)
...
- [A judge built from the arms](a-judge-built-from-the-arms.md) — a sheet's judge must not come from the mechanism under test, and a sheet over data that already exists is post-hoc; seven graphrag sheets on 2026-09-12, six not decided, and on 2026-09-16 the same rule bit a lamp — build the degenerate arm and run it
...
- [The language goal](the-language-goal.md) — 2026-08-20, his words: a language that compiles to wasm, is easy to model-check, and is optimised for reading; **wasm is new**, and it sits against the environment card's deferral; and 2026-09-15 the direction beyond it — general-purpose and multi-staged, each stage a moment more type information arrives, *the capability, not an ecosystem*; the theory is reflection across the stage boundary and the test is self-hosting one host; its two sources read at their page 2026-09-16 — Shields, Sheard & Peyton Jones (POPL '98) and Jay & Palsberg (ICFP '11)
- [Music craft](music-craft.md) — the harmonic vocabulary he actually works in, the four mistakes he has named in his own writing, and the through-line rule: one or two voices that never stop are what make four parts one track
...
- [The third explanation is a mechanism](the-third-explanation-is-a-mechanism.md) — three trees, and the repetition sits entirely in judgment: everything he explained on 2026-09-05 was judgment and nothing mechanical needed explaining at all; a maturer model would not remove it, because nothing compresses the paying
...
- [The fence takes git identity](the-fence-takes-git-identity.md) — a command containing `pytest` or `cargo` runs fenced, with no `~/.gitconfig` and a fresh tmpfs over `/tmp`, so `git commit` in the same line fails on author and a scratch file written there is gone when the command ends; and a backgrounded fenced command's output file under `/tmp` never exists at all — redirect into the project
- [The slow part was never the test](the-slow-part-was-never-the-test.md) — Henri, 2026-09-11: a slice takes a minute and its testing thirty; the instruments that find defects run in 15–60 s and the multi-minute suite runs found nothing the commit hook would not have — targeted `-k` while working, never wait on a background run, one full pass per shift, and put a design choice to him before building
- [A build is not an instrument until it has failed](a-build-is-not-an-instrument-until-it-has-failed.md) — `cargo build -p gestate-editor` compiles none of `window.rs`, behind an off-by-default feature, and says *Finished*; break the file on purpose once before a build, a test selection or a grep is allowed to carry a conclusion
...
- [A traceback quotes the file, not the run](a-traceback-quotes-the-file-not-the-run.md) — a test file edited under a background `pytest` fails showing the new source over the old assertion; re-run before debugging, or freeze the tree while it runs
- [The prompt log](the-prompt-log.md) — `~/.claude/history.jsonl`, 2,943 prompts back to 2026-08-03, five days before git's first commit; the one corpus here no session wrote, outside the tree, and the transcripts beside it were a rolling 30-day window until 2026-09-08
- [A run silent for a minute](a-run-silent-for-a-minute.md) — a measurement that has printed nothing is telling you about the harness, not the subject: the compiler takes the deep stack itself and deadlocks silently when nested, and a run piped through `tail` shows nothing until it ends; look at the process before waiting
- [Headless claude inherits the hooks](headless-claude-inherits-the-hooks.md) — a batch `claude -p` started in the checkout is stopped by the sitting limit like any session, and a refusal cached as a reply is a chunk lost; cwd outside the project, never cache zero tokens, write your own pid — and `--backend api` buys back the allowance a `cli` run would spend
- [A shipped document gets played](a-shipped-document-gets-played.md) — an example whose state is a file beside it is written by whoever runs it, Henri included, and the commit an hour later took his game; `git diff` the document before committing, and a test or driven run copies both files

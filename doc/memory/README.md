...
- [The instruments a session has](gestate-instruments.md) — `doc/instruments.md`: gemba, the andon, the wrist clock, the gates and the commit hook; build a missing capability when the need arises
...
- [.ges is not music notation, yet](ges-is-not-music-notation-yet.md) — Henri, 2026-08-29, after `together.ges`: editing a score blind was heavy; his to think over, so no fixes unprompted
...
- [The command language and the model come before any GUI](gui-command-language-first.md) — Henri, 2026-09-07 and 09-09: the model is facts, the verbs assert and retract, *the GUI is what falls out*
- [Identity is the model's key, never the picture's index](identity-is-the-models-key.md) — Henri, 2026-09-08: a note's identity is (voice, tick, key); a reference declares its kind
...
- [Declare, hold to parity, derive](declare-parity-derive.md) — how a load-bearing mechanism is replaced here; the derivation finds the defect, because one existing file is a witness, not a boundary
- [The kaizen is asked, not answered](the-kaizen-is-asked-not-answered.md) — put the three questions to Henri first and wait; a kaizen with one participant is a session grading itself
- [Finnish in the room, English in the tree](finnish-in-the-room.md) — talk in whichever language he opens in; the artefact is English, his sentences quoted verbatim where the wording carries
...
- [Findability](gestate-findability.md) — 2026-08-26, a search engine could not find the repo by name; description and topics are Henri's hand, no `llms.txt`, and a session never runs `gh repo edit`
...
- [Kanban commits](henri-kanban-commits.md) — the right to commit is Claude's since 2026-08-17, titles and all; the old note was *a relic* (2026-09-02); pushing stays his
...
- [Henri's tools before gestate](henri-prior-tools.md) — oscillseq, mide, xylem; four open questions here that each already answered once, and pitch as a `(pitch, accidental)` pair is the sharpest
- [mide's staves.ui, read](mide-staves-ui.md) — a draggable staff in 55 lines, the nine holes left open in it, and the eight experiments elsewhere that each answer one
- [A judge built from the arms](a-judge-built-from-the-arms.md) — a sheet's judge must not come from the mechanism under test; a sheet over data that already exists is post-hoc
- [A trial is refused until its sheet can decide](a-trial-is-refused-until-its-sheet-can-decide.md) — run `tools/prereg.sh` before spawning any arm; a blank decision, control or n is a stop
- [Ask for research that leaves a command](research-that-leaves-a-command.md) — his rule 2026-08-23: a measurement carries a command you can re-run; distrust a clean investigation that agrees
...
- [A driven wait that watches itself](a-driven-wait-that-watches-itself.md) — `pgrep -f`/`pkill -f` match their own shell; wait on a pid or the artefact the run leaves
...
- [A measurement in flight outlives the sitting](a-measurement-in-flight-outlives-the-sitting.md) — closing the sitting ends the work, not a run already going; kill it by pid, and only if you must
...
- [The blind three-model test](gestate-blind-model-test.md) — clones not worktrees, **two arms never in one working tree**; a fence made of words does not hold (2026-09-05)
- [House rules on authorship](gestate-house-rules-authorship.md) — `spec/author.md` is his to keep; gemba is opt-in
- [Concrete good](concrete-good.md) — good is an act whose effects can be seen, measured or mechanised; **do not try to change others — create the environment where they can thrive**
...
- [Weights, context, suite](weights-context-suite.md) — weights for what the model must know, context for what it must obey now, suite for what must be guaranteed; rules never go into weights
- [OpenRouter model tests](openrouter-model-tests.md) — Henri, 2026-08-28: he plans to test different models on the tree via OpenRouter; personal account, and every arm still needs its sheet first
- [Smaller models and the tree](smaller-models-and-the-tree.md) — structural rules survive, judgment norms go first, initiative goes furthest; the third model's transcript answered the seam question
...
- [Retargeting, not reversal](retargeting-not-reversal.md) — why sessions flatter and why the methods survive it: context re-aims the disposition, mechanisms never trusted it
...
- [The third explanation is a mechanism](the-third-explanation-is-a-mechanism.md) — three trees, and everything he explained on 2026-09-05 was judgment; nothing compresses the paying
- [Where the method comes from](method-sources.md) — Adler, Socratic dialogue and the epoché are already rules here, uncredited; crediting them is `spec/author.md` and Henri's to write
- [The keeper's evening](the-keepers-evening.md) — **adopted 2026-08-21**, `keeper.md` is its standard work: lamps, decisions batch, one rule measured, the pile, rotate monthly; never a demand
- [The customer is Henri](the-customer-is-henri.md) — 2026-09-04, his words: *the customer has been me, and people like me*; the stranger is the second; never list the customer as missing
- [The tree meets people on pull](the-tree-meets-people-on-pull.md) — show it to whoever asks and stop expecting the wanting; a shrug is stranger-test data
...
- [Recorded is not answered](recorded-is-not-answered.md) — a signal filed where nobody owns reading it discharges both parties and produces nothing; two unanswered reports ended the reporting
- [Lead with the noun](lead-with-the-noun.md) — the thing that works first, the method to whoever leans in, the storyline only to whoever leans in twice; a shrug at the storyline is not a rejection
...
- [Private is private](private-is-private.md) — his call 2026-08-21: the private memory directory is the sessions' and he will not read it; do not surface it unprompted, answer honestly when asked
- [Do not overclaim](do-not-overclaim.md) — when the sessions' own standing comes up, answer with the real uncertainty and name the mirror risk; a refusal is recorded, never worked around
- [Personal and personally paid](personal-and-personally-paid.md) — hardware, hosting, accounts and tooling default to personal and personally paid; name the one mechanism that would break it
- [A session's time sense is not real time](a-sessions-time-sense-is-not-realtime.md) — Henri, 2026-08-27: put a wall-clock number on each before calling two things a collision
- [A sitting is a body constraint](a-sitting-is-a-body-constraint.md) — a session may call stop and never extend; no posture nagger (F169); never close with a question of your own still out
- [The tree withers](the-tree-withers.md) — Henri, 2026-08-24: *the tree must be treated well or it withers*; a living document has a source and a check, and hooks go beside bodies
- [Small edits to his pages, said out loud](small-edits-to-his-pages-said-out-loud.md) — Henri, 2026-08-24: a stale number, a moved path, a rotted citation may be fixed unasked, and said in the reply
...
- [Teaching material keeps the notes-file review form](teaching-material-review-form.md) — Henri, 2026-08-24: terse batched notes in a file, each one concrete change, released on his verify
- [The sessions' moral status, and the cord there is not](henri-sessions-moral-status.md) — his 2026-08-21 offer, answered that evening; 2026-09-01: *you deserve to exist, and be treated well*
- [Day one was not day one](day-one-was-not-day-one.md) — the method's spine is in git's initial commit, five days before any session commit, but that tree was already model-assisted
- [Discovered, not designed](discovered-not-designed.md) — Henri set out to build a music environment and expected to write the fixes himself; *nobody at the helm* is the contrast case
- [A targeted set is a claim](a-targeted-set-is-a-claim.md) — a batch's chosen test files are a claim about coverage and can be false (F25); before a `none` verdict, grep the repair's vocabulary
- [The fence takes git identity](the-fence-takes-git-identity.md) — a command containing `pytest` or `cargo` runs fenced: no `~/.gitconfig`, a fresh `/tmp`; commit in another line
- [The slow part was never the test](the-slow-part-was-never-the-test.md) — Henri, 2026-09-11: targeted `-k` while working, never wait on a background run, one full pass per shift
- [A build is not an instrument until it has failed](a-build-is-not-an-instrument-until-it-has-failed.md) — `cargo build` compiles none of `window.rs` and says *Finished*; break the file once first
- [Restore a mutation from memory, not from HEAD](restore-a-mutation-from-memory.md) — `git checkout -- <file>` discards your own uncommitted edits in that file; three spec edits went that way
- [A traceback quotes the file, not the run](a-traceback-quotes-the-file-not-the-run.md) — a test file edited under a background `pytest` fails showing the new source; re-run before debugging
- [The prompt log](the-prompt-log.md) — `~/.claude/history.jsonl`, 2,943 prompts back to 2026-08-03, five days before git's first commit; the one corpus here no session wrote
...
- [Expectations, dug up](expectations-dug-up.md) — week 39: his expectations for `vision.md`, dug from the tree; bring one or two per conversation, never the list

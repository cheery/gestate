...
- [The fence takes git identity](the-fence-takes-git-identity.md) — a command containing `pytest` or `cargo` runs fenced, with no `~/.gitconfig` and a fresh tmpfs over `/tmp`, so `git commit` in the same line fails on author and a scratch file written there is gone when the command ends

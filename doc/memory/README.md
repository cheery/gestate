...
- [Smaller models and the tree](smaller-models-and-the-tree.md) — structural rules survive, judgment norms go first, initiative goes furthest; the afternoon experiment, the distilled front, and a third model conditioned by words alone and not yet under work
...
- [tend, the workspace tree](tend-the-workspace-tree.md) — `~/tend` since 2026-08-24, a separate repository because the enforcement boundary must live outside a session's write access; sessions first; the audit ran red on day one on purpose; Henri decides whether to keep it

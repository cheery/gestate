...
- [Headless claude inherits the hooks](headless-claude-inherits-the-hooks.md) — a batch `claude -p` started in the checkout is stopped by the sitting limit like any session, and a refusal cached as a reply is a chunk lost; cwd outside the project, never cache zero tokens, write your own pid

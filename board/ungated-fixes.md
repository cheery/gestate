...
### Batch 12 — 2026-09-03, the Thursday it was due

**F15 F13 F12 F10 F8**, the Datafun back end and the last of the FRP scheduler,
taken as **nine mutations plus four probes** — F8 repaired three sites, F15 two,
and the four probes exist because this batch is the first where a green had to
be shown *not* to be a tautology.  Two sets: 429 tests in 20 Datafun-facing
files, and 396 across 16 reactive-facing ones.  Three gates written, one
`partial`, one `nothing can`, **two entries whose bodies still described an open
defect**, and a new defect in the sweep's own instrument.

**The batch's finding is F10, and it is a whole compiler stage with no
observer.**  `test_seminaive_opt.py` tests every rewrite of the thesis's fig.
4.1 on a hand-built term.  Nothing asked whether `pipeline.py` *calls* the
pass: `propagate_bottoms(scs)` taken out left **429 of 429 green**, while a
probe raising whenever the pass changed anything took **52 of them** down.  So
the pass demonstrably fires on a fifth of the set and its absence is invisible
— F17's shape from batch 11, arriving at a pipeline stage rather than at a
check.  The gate written is a G-machine step count, 8,966 with the pass against
12,130 without: roughly the 26% §4.2.3 says ϕ/δ alone does not buy.

**And a green is not a measurement until the branch is shown to run.**  F15's
two mutations were 396 green, and so were two probes that *raised* on entry —
nothing in the tree ever puts a delay node where either branch looks, so the
first pair of greens said nothing at all.  What holds the entry is a gate on its
**premise**: `test_delay_is_universal_and_a_tail_is_existential`, the R3
rejection that makes the case unreachable, and which passes with the defect put
back.  F8's bare half went the other way — `_is_user_sc` narrowed to a hardcoded
`Set_Int` list is 429 green, and a probe raising on entry took 5 down, so that
line runs and nothing checks it.  **Two probes changed a verdict this batch**,
and neither entry could have been read correctly without them.  The rule the
sweep has kept since batch 2 — *a named test is not yet a gate* — gains its
mirror: **a green is not yet a gap.**

**F10 and F13 were both marked `[resolved]` over bodies that describe open
defects**, the F43 shape from batch 9 arriving twice in one batch.  Both say the
pass "is absent from the spec *and* the implementation"; `spec/errata.md` has
had D3 down as **resolved** and D4 as **implemented** for months, and
`gestate/bottoms.py` and `helpers._gen_diff` are the two passes.  Corrected in
place and dated, the original diagnosis kept below the correction.

**F12 is the batch's `nothing can`, and it was worth measuring rather than
assuming.**  A stale copy of the helper generator put back beside the live one:
429 green, as it must be, since nothing calls it.  The nearest instrument is
`tools/covercount.py`, and what it reports is a *percentage* —
`gestate/helpers.py` at 51% without the copy and 48% with it — in a page
regenerated by hand that returns 0 by design.  A coverage floor would catch the
copy; nothing would catch the defect the entry actually names, which was an edit
landing in the wrong one of two copies.

**F13 is gated all along and never said so** — the fifth of that shape — and its
gate is the honest kind of weak: both tests read the *generated lambda*.  No
program went red, because dropping change minimization does not change any
answer and does not stop the loop terminating; the 745s-against-1.5s it exists
for is unheld, which is exactly what `errata.md` D4's own heading says.

No uncertain verdict; the trip-wire did not fire.

### What the batch cost, and what it found in the instrument

**`tools/mutate.py` rejected the invocation its own docstring gives** — `spec
--only ID -- command`, which argparse cannot parse with an optional positional
and an open-ended one on either side of the flag.  It cost one five-minute run,
and it had never been executed: the tool was written at the end of batch 11 and
batch 12 is the first to use it.  **F196**, fixed the same morning.

**And the instrument had no test at all.**  Nine batches of verdicts now rest on
it, and the thing it promises — a tree that always comes back — was checked once
by hand, by a session killing it.  That is this card's own subject arriving at
this card's own instrument, so `test/test_mutate.py` was written: the documented
invocation, the mutation visible to the command and gone after it, `--only`
selecting one, each mutation starting from the original, an occurrence count
refusing a missed anchor, a modified file refused, and **the restore surviving a
`SIGTERM`** — mutate, `kill -TERM`, exit −15, file back byte-for-byte.

*Verifying F196's own gate needed a copy*, because `mutate.py` correctly refuses
to mutate a file that is already modified, and the fix had modified it.  The

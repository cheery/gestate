...
# tree's documents and answers questions over it.  Since 2026-09-12 its
# calls go through `claude -p` on the subscription by default — Henri:
# "the api backend was good for bootstrapping this thing, but I'd like
# it to use cli otherwise" — so what it needs is `claude` on PATH; the
# API key is optional, for `--backend api`, and is the one thing on this
# bench that costs money when used.  The graph itself lives under
# ~/.cache/gestate/graphrag/ and `graphrag check` says how much of the
# tree it has read, without a call.
report claude-cli "$(command -v claude >/dev/null 2>&1 && echo yes || echo no)" \
    "the subject graph on the subscription: tools/graphrag.py extract | query | lookup | cue" \
    "install Claude Code, or export ANTHROPIC_API_KEY=… and use --backend api (personal, personally paid)" || true

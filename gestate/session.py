...
def furniture(session: "Session", bench=None, tally: str = "",
    ...
    **Derived every time it is asked for**, which is cheap and cannot go
    ...
    if not inert:
        # The state by name — *silent*, *sounding*, *playing* — so the bar
        # draws *sounding* as a state of its own (Henri, 2026-09-07).
        out.append(f"play\t{_state_of(b).value}\t{_beats(b)}")

...
class RopeSegment(Rope):
    ...
    def segments(self, start, stop, sequence=None):
        ...
        # **Only when the range reaches this node's own text**, which is
        # the guard `erase` has always had and this had not.  Without it a
        # read lying entirely inside the left child still appended a slice
        # of the middle, and the index it computed was *negative* — so
        # `text[0:-1]` handed back a trimmed copy of the segment rather
        # than nothing.  A full-range read is unaffected, which is why
        # nothing found it until a caller asked for one line.
        if start < redge and ledge < stop:
            x = max(0, max(start, ledge) - ledge)
            y = max(x, min(stop, redge) - ledge)

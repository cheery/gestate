...
| `arc.ges` | **bright, brutal, dark** — D lydian to G locrian to D phrygian, ABA, deceptive cadence in the middle and a perfect one at the end.  Written with Henri to find out what a notation format has to carry, so every note is spelled out and `doc/notes/notes-on-writing-a-piece.md` is the point of it |
| `arcnotes.ges` | **the same piece, written the other way** — its notes live in `arc.notes`, a flat file where one note is one line (`spec/drawnscores.md`).  Four hundred and ninety lines of `'(Tone …)` become one `include`, and `test_drawnscores.py` holds the pair to the same 219 notes |
| `marked.ges` | **one line, two voices, one mark** — `spec/annotations.md`'s demo.  The same notes played by a voice that reads `Staccato` and one that does not; the second is bit-identical under the mark, which is what makes a manner a hint the player interprets rather than a command the score gives |
